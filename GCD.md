...
- 当前代码执行到 **dispatch_sync()** 时，**当前线程会阻塞**，等到 **dispatch_sync()** 传入的任务（Block）执行完毕，函数才会返回，代码才会往下走
- 换句话说，同步执行会要求当前线程**立即执行**该任务
...
- 由于是**同步**添加的**任务1**，主线程会**阻塞**，等待 **dispatch_sync()** 中的 **任务1**执行完毕后，才会继续执行后面的其他任务
- **任务1** 需要等前面的其他任务执行完毕后，才能执行，因为**主队列**是**串行队列**，这样就造成了**死锁**
- **其他任务**在等待**任务1**，**任务1**在等待**其他任务**
...
- 在**子线程**中使用主队列同步添加任务
  - 不会卡住主线程，可以放心地等到之前的任务执行完之后，执行任务1
  - 和异步执行主队列基本等效
- 在主线程中使用**其他队列**添加任务
- 改为异步执行
...
异步执行具备开启新线程的能力，由于是**串行队列**，串行队列只能一个一个执行，所以只开一个子线程，**任务都在子线程**上执行，**主线程不会阻塞**，相对应的，UI不会卡顿

![](Images/image-20221119053025782.png)



### 异步执行 + 并发队列

两者都具备多线程的条件，所以将开启多个子线程，同时执行队列里的任务，多任务同时进行，同样的，任务都在子线程上执行，主线程不会阻塞

![image](Images/Snipaste_2022-11-19_05-44-25.png)

这里有几个点值得注意下

- 异步执行的并发队列执行顺序是不确定的

- 即使把任务1放前面，实际执行的时候不一定任务1就第一个开始
...
由于是**异步执行**，添加完一个任务，不用等待任务的结束，**代码可以直接往下走**

这也是和同步执行的区别，这样做就**不会造成死锁**

虽然没有死锁，由于是**主队列**（主队列是**串行队列**）,**所以没有开新的子线程**，主线程在**执行任务**的时候（注意**不是添加任务**，死锁的情况是添加任务就阻塞了）**会阻塞**，**UI会卡住**
